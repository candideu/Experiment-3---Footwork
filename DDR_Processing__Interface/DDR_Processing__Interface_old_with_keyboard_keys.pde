...
Based on "DDR in Processing" by Jon Castro (3/24/17): https://jon-castro.itch.io/ddr-in-processing
Music visualizer adapted from "Starburst-Music-Viz" by Andre Le: https://github.com/andrele/Starburst-Music-Viz/blob/master/andreMusicViz.pde
Arduino to Processing framework based "Read Cap IMU" and "Read All Pins Launch on Touch" by Kate Hartman & Nick Puckett (DIGF 6037 Creation & Computation): https://github.com/DigitalFuturesOCADU/CC2020/blob/main/Experiment3/codeExamples/ArduinoToProcessing/CSV/SendItAll
...
void move(){
  if (keyPressed) {
    if (keyCode == keys && x < 200 && x >= 150 && press == false) { // (keyCode == keys && x < 50 && press == false)
      score += 100 * combo;
      combo++;
      press = true;
      hit = true;
